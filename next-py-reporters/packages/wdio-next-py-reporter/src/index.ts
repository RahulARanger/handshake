...
export default class NextPyReporter extends ReporterContacts {
		...
		this.lock.acquire(
			...
			async (done) => {
				...
				const resp = await fetch(feedURL, {
					...
					resp.ok ? undefined : new Error(resp.statusText),
					await resp.text()
				);
			},
			(er, _text) => {
	...
	extractRegistrationPayloadForTestEntity(
		...
		const { title, file, tags, description } = suiteOrTest as SuiteStats;
		...
		const payload = {
			...
			file:
				sanitizePaths([
					file || this.currentSuites.at(-1)?.file || "",
				]).at(0) ?? "",
	...
	extractRequiredForEntityCompletion(
		...
		const { errors, error, state: _state } = suiteOrTest as TestStats;
		const state = _state as undefined | string;
