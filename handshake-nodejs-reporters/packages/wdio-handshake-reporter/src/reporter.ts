...
export default class HandshakeReporter extends ReporterContacts {
	...
	extractRegistrationPayloadForTestEntity(
		...
		const payload = {
			...
			file:
				sanitizePaths([
					file ||
						this.currentSuites.at(this.expectedIndex(snapshot, -1))
							?.file ||
						"",
				]).at(0) ?? "",
			tags:
	...
	onSuiteStart(suite: SuiteStats): void {
		...
		this.supporter.registerTestEntity(suite.uid, () =>
			this.extractRegistrationPayloadForTestEntity(
				suite,
				"SUITE",
	...
	addTest(test: TestStats, note?: number): void {
		...
		this.supporter.registerTestEntity(test.uid, () =>
			this.extractRegistrationPayloadForTestEntity(
				test,
				"TEST",
	...
	onSuiteEnd(suite: SuiteStats): void {
		...
		this.supporter.updateTestEntity(() =>
