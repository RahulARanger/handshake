...
export default class HandshakeJestReporter implements Reporter {
	...
	private trackSuites: Map<string, LeafEntity> = new Map();

	private trackParentStatus: Map<string, Set<string>> = new Map();

	private trackEntities: string[] = [];
	...
	constructor(globalConfig: Config.GlobalConfig, options?: any) {
		...
		const logLevel: undefined | Level =
			...
			await this.reporterSupport.registerParentHierarchy(
				() =>
					testCaseStartInfo.ancestorTitles.map((suite) => {
						...
						if (!this.trackParentStatus.has(store))
							this.trackParentStatus.set(store, new Set());
						if (parent)
							this.trackParentStatus.get(parent)?.add(store);
						if (currentFile?.get(store))
							return currentFile.get(store) as string;
				...
				(resp) => {
					...
					response.forEach((id, index) =>
						currentFile.set(ancestors[index], id)
					);
				}
			);
		}
	...
	async onTestFileResult(
		...
		this.trackSuites.delete(test.path);
