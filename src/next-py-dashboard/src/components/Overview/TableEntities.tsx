...
import DetailsOfRun from "@/types/testRun";
...
export default function TestEntities(props: {getSuites: string, getTestRun: string}): ReactNode{
    const {data} = useSWRImmutable<SuiteDetails[]>(props.getSuites)
    const {data: testRun} = useSWRImmutable<DetailsOfRun>(props.getTestRun)
    if(!data || !testRun) return <Skeleton width={200} height={150}/>
    const testStartedAt = dayjs(testRun.started)
    const sliced = data.slice(-10, data.length).reverse().map((testDate) => parseTestEntity(testDate, testStartedAt))
    ...
    return (
        <div className="ag-theme-alpine-dark" style={{height: 270, width: 550}}>
            <AgGridReact
                rowData={sliced}
